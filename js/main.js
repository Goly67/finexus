...
// Demo properties for fallback
  window._demoProperties = [
    {
      ...
      area: '250 sqm',
      price: '15,000,000',
      desc: 'Beautiful modern villa with ocean views, perfect for families. Features include spacious living areas, modern kitchen, and private garden.',
    ...
    },
    {
      ...
      area: '85 sqm',
      price: '3,500,000',
      desc: 'Convenient downtown apartment with city views. Close to shopping centers and public transport.',
    ...
    },
    {
      ...
      area: '120 sqm',
      price: '8,000,000',
      desc: 'High-end condominium with resort-style amenities including swimming pool, gym, and 24/7 security.',
        ...
        <div class="property-card reveal" style="transition-delay:${i * 0.08}s" onclick="openModal('${p.id}')">
          <div class="property-img">
            <img src="${imgUrl || 'https://images.unsplash.com/photo-1560184897-ae75f418493e?w=600&q=80'}" 
                 alt="${p.name}" onerror="this.src='https://images.unsplash.com/photo-1560184897-ae75f418493e?w=600&q=80'">
  ...
  function convertDriveUrl(url) {
    if (!url) return '';
    // Handle Google Drive URLs in various formats
    let fileId = null;
    ...
    if (fileId) {
      // Use Google Drive thumbnail format (often more reliable for images)
      return `https://drive.google.com/thumbnail?id=${fileId}&sz=w800`;
    }
    
    // If not a Google Drive URL, return as-is (it might be a direct image URL)
    return url;
  }

  // ── UPDATE LOCATIONS ──
  function updateLocations(props) {
    // Get unique locations, extract city/country name (after last comma)
    ...
    if (locationsUl) {
      const footerHtml = uniqueLocations.map(location => 
        `<li><a href="#properties" data-location="${location}">${location}</a></li>`
      ).join('');
      locationsUl.innerHTML = footerHtml;
    }

    // Update search location dropdown
    ...
    // Re-attach event listeners for the new footer links
    attachFooterListeners();
  }

  // ── FILTER ──
  function parsePrice(value) {
    // Normalize string like "15,000,000" or "15000000" to number
  ...
  function filterProperties() {
    ...
    if (price === 'Under ₱5M') {
      priceBounds.max = 5_000_000;
    } else if (price === '₱5M – ₱15M') {
      priceBounds.min = 5_000_000;
      priceBounds.max = 15_000_000;
    } else if (price === '₱15M – ₱30M') {
      priceBounds.min = 15_000_000;
      priceBounds.max = 30_000_000;
    } else if (price === '₱30M+') {
      priceBounds.min = 30_000_000;
    }
  ...
  function openAllPropertiesModal() {
    ...
    if (!_allProperties || _allProperties.length === 0) {
      ...
      // Group properties by type (normalize case)
      let grouped = {};
      _allProperties.forEach(p => {
        ...
        console.log('Property:', p.name, 'Original Type:', type, 'Normalized Type:', normalizedType, 'Featured:', p.featured);
      ...
      // Sort each group by featured first
      ...
      if (featured.length > 0) {
        ...
        html += '<div class="property-grid" id="carousel-featured">' + featured.map(p => generatePropertyCard(p)).join('') + '</div>';
      }

      // Define all property types to ensure sections exist
      const allTypes = ['Villa', 'House', 'Condo', 'Apartment', 'Commercial', 'Lot Only', 'Vacant Lot'];
      
      // Add any additional types found in data
      Object.keys(grouped).forEach(type => {
        if (!allTypes.includes(type) && type !== 'Other') {
          allTypes.push(type);
        }
      });
      
      // If there are "Other" properties, add them at the end
      if (grouped['Other']) {
        allTypes.push('Other');
      }

      // Create sections for each type (including featured properties in their type sections)
      allTypes.forEach(type => {
        ...
        if (items.length > 0) {
          ...
          html += `<div class="no-properties" style="text-align: center; padding: 40px; color: var(--mid);">
            <div style="font-size:2.5rem;margin-bottom:12px;">🏠</div>
            <p style="font-size:1.1rem;">No ${type.toLowerCase()} properties available</p>
  ...
  function initializeCarouselPagination(typeId) {
    const carousel = document.getElementById(`carousel-${typeId}`);
    const pages = document.getElementById(`pages-${typeId}`);
    if (!carousel || !pages) return;

    const cards = Array.from(carousel.querySelectorAll('.property-card'));
    if (!cards.length) return;

    pages.innerHTML = cards.map((card, idx) => `
      <button class="page-btn" data-type="${typeId}" data-index="${idx}">${idx + 1}</button>
    `).join('');
    ...
    const firstBtn = pages.querySelector('.page-btn');
  ...
  function openModal(id) {
    ...
    document.getElementById('modalDesc').textContent = p.desc || 'Detailed description not available. Contact us for more information about this property.';
  ...
  function setupCarouselIndicators() {
    ...
    // Show carousel controls for multiple images
  ...
  document.getElementById('modalImg').addEventListener('touchstart', function(e) {
    touchStartX = e.changedTouches[0].screenX;
  });

  document.getElementById('modalImg').addEventListener('touchend', function(e) {
  ...
  function attachFooterListeners() {
    document.querySelectorAll('.footer-col a[data-type], .footer-col a[data-location]').forEach(link => {
      link.addEventListener('click', (e) => {
        ...
        let filtered = _allProperties.slice(); // copy array
        if (type) {
          filtered = filtered.filter(p => p.type === type);
        }
        if (location) {
          filtered = filtered.filter(p => p.location && p.location.split(',').pop().trim() === location);
        }
        renderGrid(filtered);
      });
    });
